...
def building_special_fn(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        multi_word_no_land + building_special + COMMON_ENDINGS, flags=re.IGNORECASE
    )
    return [(m.start(), m.end(), BUILDING) for m in pattern.finditer(row.text)]
...
def company_name(row: Any) -> EntityList:
    ...
    pattern = re.compile(r"[a-z\(\)'\s&]+" + company_type_regex, flags=re.IGNORECASE)
...
def mill(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        r"(?<=[0-9]|,|\))\s([a-z][a-z'\s]+mill)(?=,| and|$|;|:)", flags=re.IGNORECASE
    )
...
def word_building_number(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        r"(block|building) ([a-z]([0-9]?)|\d+|)(?=,)", flags=re.IGNORECASE
    )
    return [(m.start(), m.end(), BUILDING) for m in pattern.finditer(row.text)]
...
def meadows_regex(row: Any) -> EntityList:
    """Mark all matches of a regular expression within the document text as a span."""
    pattern = re.compile(
        r"(?<=[0-9]\s)[a-z][a-z'\s]*meadow(s)?(?=,| and|$|;|:)", flags=re.IGNORECASE
    )
    return [(m.start(), m.end(), STREET_NAME) for m in pattern.finditer(row.text)]
...
def park_roads(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        r"\b[a-z][\sa-z']*park" + r"(?=,| and| \(|$)", flags=re.IGNORECASE
    )
    return [(m.start(), m.end(), STREET_NAME) for m in pattern.finditer(row.text)]
...
def wharf_road(row: Any) -> EntityList:
    ...
    return [
        (m.span(1)[0], m.span(1)[1], STREET_NAME) for m in pattern.finditer(row.text)
    ]
...
def adjective_space_number_words_roadtitle_fn(row: Any) -> EntityList:
    ...
    return [
        (m.span(1)[0], m.span(1)[1], STREET_NUMBER) for m in pattern.finditer(row.text)
    ]
...
def no_road_near(row: Any) -> EntityList:
    ...
    return [
        (m.span(2)[0], m.span(2)[1], STREET_NUMBER) for m in pattern.finditer(row.text)
    ]
...
def number_before_building(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        r"^([a-z]?\d+[a-z0-9\-\.]*),?\s+[^,]*?" + building_names, flags=re.IGNORECASE
    )
    match = pattern.search(row.text)
...
def flat_letter(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        UNIT_PATTERN + r"\s([a-z][0-9\.\-]*)(?=,)", flags=re.IGNORECASE
    )
    matches = []
...
def flat_letter_complex(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        UNIT_PATTERN + r"\s([a-z](\d|\.|-)[a-z0-9\-\.]*)", flags=re.IGNORECASE
    )
...
def flat_unit(row: Any) -> EntityList:
    ...
    pattern = re.compile(
        r"(apartment|flat|penthouse|unit|plot)\b(?!being)", flags=re.IGNORECASE
    )
