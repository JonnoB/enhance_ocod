...
def clean_raw_data(path):
    ...
    gt_data = gt_data_raw.loc[
        gt_data_raw["type"] == "annotation",
        [
            "input:datapoint_id",
            "start",
            "end",
    ...
    gt_data = gt_data.rename(
        columns={"input:text": "property_address", "input:datapoint_id": "datapoint_id"}
    )
...
gt_test_data.to_csv(
    "data/enhanced_ocod_data_and_gt/ground_truth_test_set_labels.csv", index=False
)
...
gt_dev_data.to_csv(
    "data/enhanced_ocod_data_and_gt/ground_truth_dev_set_labels.csv", index=False
)
