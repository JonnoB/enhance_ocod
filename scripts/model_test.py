...
def analyze_entity_errors(y_true, y_pred, val_data, tokenizer):
    ...
    for i, (true_seq, pred_seq, data_item) in enumerate(zip(y_true, y_pred, val_data)):
        ...
        for entity_type, start, end in false_negatives:
            entity_text = get_entity_text_from_tokens(
                true_seq[start : end + 1], text, tokenizer, start, end
            )
            errors.append(
                {
                    "example_id": i,
        ...
        for entity_type, start, end in false_positives:
            entity_text = get_entity_text_from_tokens(
                pred_seq[start : end + 1], text, tokenizer, start, end
            )
            errors.append(
                {
                    "example_id": i,
    ...
    for entity_type, count in sorted(
        entity_type_errors.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    for entity_type in sorted(entity_stats.keys()):
        ...
        print(
            f"  {entity_type:15} - P: {precision:.3f}, R: {recall:.3f}, F1: {f1:.3f} (TP:{tp}, FP:{fp}, FN:{fn})"
        )

    return pd.DataFrame(errors)


def get_entity_text_from_tokens(
    token_labels, original_text, tokenizer, start_pos, end_pos
):
    """
...
def get_entity_text_from_positions(text, token_positions, tokenizer, label_sequence):
    ...
    try:
        ...
        encoding = tokenizer(
            text, add_special_tokens=True, truncation=True, return_offsets_mapping=True
        )
        ...
        if actual_start_token < len(offset_mapping) and actual_end_token < len(
            offset_mapping
        ):
...
def analyze_entity_errors_improved(y_true, y_pred, val_data, tokenizer):
    ...
    for i, (true_seq, pred_seq, data_item) in enumerate(zip(y_true, y_pred, val_data)):
        ...
        for entity_type, start, end in false_negatives:
            token_positions = f"{start}-{end}"
            entity_text = get_entity_text_from_positions(
                text, token_positions, tokenizer, true_seq
            )
        ...
        for entity_type, start, end in false_positives:
            token_positions = f"{start}-{end}"
            entity_text = get_entity_text_from_positions(
                text, token_positions, tokenizer, pred_seq
            )
...
def show_human_readable_analysis(errors_df):
    ...
    if len(example_2_errors) > 0:
        ...
        print("\nThis shows:")
        print("  - You labeled: '39', 'to', '44', 'chapel', 'street' all as CITY")
        print(
            "  - Model predicted: '39'=street_number, 'to'=nothing, '44'=street_number, 'chapel street'=street_name"
        )
        print("  - The model is probably correct here!")
    ...
    entity_issues = (
        errors_df.groupby(["entity_type", "error_type"])
        .size()
