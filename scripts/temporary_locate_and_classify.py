...
def filter_contiguous_numbers(number_list, number_filter):
    ...
    if number_filter == "odd":
        ...
        out = [
            x for x in number_list if isinstance(x, (int, str))
        ]  # Keep original type if not int for 'all'
    return out
...
def expand_dataframe_numbers(df2, column_name, print_every=1000, min_count=1):
    ...
    for i in range(0, df2.shape[0]):
        ...
        if dataframe_len > min_count or (dataframe_len == 1 and min_count == 1):
            ...
            tmp[column_name] = [
                df2.loc[i, column_name]
            ]  # Ensure it's still a list or single value
    ...
    if temp_list:
        ...
        out = pd.DataFrame(
            columns=df2.columns.tolist() + [column_name]
        )  # Return empty DataFrame with correct cols
...
def preprocess_expandaded_ocod_data(ocod_data, postcode_district_lookup):
    ...
    ocod_data = ocod_data.merge(
        postcode_district_lookup, "left", left_on="postcode2", right_on="postcode2"
    )

    ocod_data["street_name"] = ocod_data["street_name"].str.replace(
        r"^ +| +$", r"", regex=True
    )
...
def add_missing_lads_ocod(ocod_data, price_paid_df):
    ...
    lad_lookup = (
        price_paid_df[["district", "lad11cd"]]
        .dropna()
        .groupby(["district", "lad11cd"])
        .size()
        .reset_index()
    )
    lad_lookup.rename(columns={0: "counts"}, inplace=True)
    lad_lookup = (
        lad_lookup.sort_values("counts", ascending=False)
        .groupby("lad11cd")
        .first()
        .reset_index()
    )
    ...
    temp[
        "lad11cd"
    ].isna().sum()  # there are no na values showing all districts now have a lad code
...
def massaged_address_match(ocod_data, voa_data, target_lad):
    ...
    ocod_data_road.loc[ocod_data_road.street_name.isna(), "street_name2"] = (
        "xxxstreet name missingxxx"
    )

    # the roads which match
    ocod_data_road["street_match"] = ocod_data_road["street_name2"].isin(
        LAD_biz.street_name2.unique()
    )

    # remove irrelevant streets
    LAD_biz = LAD_biz[
        LAD_biz["street_name2"].isin(ocod_data_road["street_name2"].unique())
        & LAD_biz["street_name2"].notna()
    ]
    # create the database table
    all_street_addresses = (
        create_all_street_addresses(LAD_biz, target_lad)
        .drop_duplicates(subset=["street_name2", "street_number"])
        .rename(columns={"street_number": "street_number2"})
    )

    # prevents matching errors caused by some arcance and horrible thing
    all_street_addresses["street_number2"] = all_street_addresses[
        "street_number2"
    ].astype("str")
    all_street_addresses["street_number2"] = all_street_addresses[
        "street_number2"
    ...
    ocod_data_road = ocod_data_road.merge(
        all_street_addresses, how="left", on=["street_name2", "street_number2"]
    )

    ocod_data_road["address_match"] = ocod_data_road["business_address"].notna() == True

    ocod_data_road.loc[
        ocod_data_road["street_name2"] == "xxxstreet name missingxxx", "street_name2"
    ] = np.nan

    return ocod_data_road


def create_all_street_addresses(
    voa_businesses, return_columns=["street_name2", "street_number", "business_address"]
):
    ...
    temp.loc[
        temp["street_number"].str.contains(r"\.", regex=True, na=False), "street_number"
    ] = np.nan
    ...
    temp_multi = temp_multi.reset_index(
        drop=True
    )  # Reset index after filtering to ensure it's contiguous for iloc or proper merging

    if (temp_multi.shape[0] > 0) and (not_multi_address.shape[0] > 0):
        # expand the dataframe according to the correct rules
        temp_multi = expand_dataframe_numbers(
            temp_multi, "street_number", print_every=10000, min_count=1
        )
        street_address_lookup = pd.concat([temp_multi, not_multi_address])[
            return_columns
        ]
...
def load_voa_ratinglist(file_path, postcode_district_lookup):
    ...
    mask_advertising = ~voa_businesses["primary_description_text"].str.contains(
        "ADVERTISING", na=False
    )
    exclude_codes = {"C0", "CP", "CP1", "CX", "MX"}
    mask_codes = ~voa_businesses["primary_and_secondary_description_code"].isin(
        exclude_codes
    )
...
def street_and_building_matching(ocod_data, price_paid_df, voa_businesses):
    ...
    temp = (
        temp_lsoa.groupby(["street_name2", "lad11cd", "lsoa11cd"])
        .size()
        .reset_index()
    ...
    ocod_data = ocod_data.merge(
        temp[["lsoa_street", "street_name2", "lad11cd"]],
        how="left",
    ...
    temp = (
        temp[temp["paon"].str.len() != 0]
        .groupby(["paon", "lad11cd"])
        .size()
        .reset_index()
        .rename(columns={0: "counts"})
    )
    ...
    temp = temp[temp["counts"] == 1].merge(pp_temp, how="left", on=["paon", "lad11cd"])

    temp = temp.rename(
        columns={
            "lsoa11cd": "lsoa_building",
            "oa11cd": "oa_building",
            "paon": "building_name",
        }
    ...
    ocod_data = ocod_data.merge(
        temp[["lsoa_building", "oa_building", "building_name", "lad11cd"]],
        how="left",
    ...
    ocod_data = ocod_data.merge(
        voa_businesses.loc[
            ~voa_businesses["building_name"].isna(),
            ["building_name", "oa11cd", "lsoa11cd", "lad11cd"],
        ]
        .drop_duplicates(subset=["building_name", "lsoa11cd"])
        .rename(
            columns={"lsoa11cd": "lsoa_busi_building", "oa11cd": "oa_busi_building"}
        ),
        how="left",
        on=["building_name", "lad11cd"],
    )
    ...
    for x in [
        ...
        ocod_data.loc[ocod_data["lsoa11cd"].isnull(), "lsoa11cd"] = ocod_data[x][
    ...
    for x in ["oa_building", "oa_busi_building"]:  # , 'lsoa_business']:
        ocod_data.loc[ocod_data["oa11cd"].isnull(), "oa11cd"] = ocod_data[x][
            ocod_data["oa11cd"].isnull()
        ]
    ...
    temp = (
        ocod_data.loc[
            (ocod_data["lsoa11cd"].notnull())
            & (ocod_data["within_larger_title"] == True),
            ["lsoa11cd", "title_number"],
        ]
        .groupby(["lsoa11cd", "title_number"])
        .size()
        .reset_index()
    )
    temp = temp[["lsoa11cd", "title_number"]].rename(
        columns={"lsoa11cd": "lsoa_nested"}
    )
    ...
    ocod_data = ocod_data.merge(
        temp[["title_number", "lsoa_nested"]], how="left", on="title_number"
    )

    ocod_data.loc[ocod_data["lsoa11cd"].isnull(), "lsoa11cd"] = ocod_data[
        "lsoa_nested"
    ][ocod_data["lsoa11cd"].isnull()]
    ...
    temp = (
        ocod_data.loc[
            (ocod_data["oa11cd"].notnull())
            & (ocod_data["within_larger_title"] == True),
            ["oa11cd", "title_number"],
        ]
        .groupby(["oa11cd", "title_number"])
        .size()
        .reset_index()
    )
    ...
    ocod_data = ocod_data.merge(
        temp[["title_number", "oa_nested"]], how="left", on="title_number"
    )

    ocod_data.loc[ocod_data["oa11cd"].isnull(), "oa11cd"] = ocod_data["oa_nested"][
        ocod_data["oa11cd"].isnull()
    ]
...
def substreet_matching(
    ocod_data, price_paid_df, voa_businesses, print_lads=False, print_every=100
):
    """ "
    ...
    filled_lsoa_list = []
    i = 1
    unique_lad_codes = ocod_data[
        ocod_data["street_name"].notnull()
    ...
    for target_lad in unique_lad_codes:
        ...
        if i % print_every == 0:
            print("lad ", i, " of " + str(round(len(unique_lad_codes), 3)))
        i = i + 1

        # subset to the relevat rows within a single lad
        missing_lsoa_df = ocod_data[
            ocod_data["street_name"].notnull()
            & ocod_data["street_number"].notnull()
            & ocod_data["lsoa11cd"].isnull()
            & (ocod_data["lad11cd"] == target_lad)
        ].copy()
        missing_lsoa_df.loc[:, "street_number2"] = (
            missing_lsoa_df.loc[:, "street_number"]
            .str.replace(r"^.*(?=\b[0-9]+$)", "", regex=True)
            .str.replace(r"[^\d]", "", regex=True)
        )
        ...
        temp_lsoa = pd.concat(
            [
                ...
                price_paid_df[
                    price_paid_df["street_name2"].isin(target_street_names)
                    & (price_paid_df["lad11cd"] == target_lad)
                ],
                # voa data added in
                voa_businesses[(voa_businesses["lad11cd"] == target_lad)],
            ]
        )[["street_name2", "street_number", "lsoa11cd", "lad11cd"]].dropna(
            axis=0, how="any", inplace=False
        )

        temp_lsoa.loc[:, "street_number2"] = (
            temp_lsoa.loc[:, "street_number"]
            .str.replace(r"^.*(?=\b[0-9]+$)", "", regex=True)
            .str.replace(r"[^\d]", "", regex=True)
        )
        ...
        temp_data = temp_lsoa[
            temp_lsoa["street_name2"].isin(target_street_names)
        ...
        temp_lsoa = create_all_street_addresses(
            temp_data, ["street_name2", "street_number", "lsoa11cd"]
        )

        for target_road in target_street_names:
            # print(target_road)
            missing_lsoa_road = missing_lsoa_df[
            ...
            if len(temp_road) > 0:
                ...
                for missing_lsoa_row in range(0, len(missing_lsoa_road)):
                    street_number_value = missing_lsoa_road.iloc[missing_lsoa_row][
                        "street_number2"
                    ]
                    safe_street_number = safe_int_convert(street_number_value)

                    if safe_street_number is not None:
                        lsoa_result = street_number_to_lsoa(
                            temp_road, safe_street_number
                        )
                    else:
    ...
    ocod_data = pd.concat(
        [ocod_data[~ocod_data["unique_id"].isin(temp_lsoa["unique_id"])], temp_lsoa]
    )
    ...
    ocod_data = ocod_data.merge(
        temp[["title_number", "lsoa_nested2"]], how="left", on="title_number"
    )
...
def voa_address_match_all_data_old(
    ocod_data, voa_businesses, print_lads=False, print_every=50
):
    """
    ...
    for target_lad in all_lads:
        ...
        matched_lads_list = matched_lads_list + [
            massaged_address_match(ocod_data, voa_businesses, target_lad)
        ]
...
def voa_address_match_all_data(ocod_data, voa_businesses):
    ...
    ocod_data_processed.loc[ocod_data_processed.street_name.isna(), "street_name2"] = (
        "xxxstreet name missingxxx"
    )
    ocod_data_processed["street_number2"] = (
        ocod_data_processed["street_number2"].astype(str).str.strip()
    )
    ...
    unique_voa_streets = all_voa_addresses_lookup[
        ["lad11cd", "street_name2"]
    ].drop_duplicates()
    unique_voa_streets["is_voa_street"] = True

    ocod_data_processed = ocod_data_processed.merge(
        unique_voa_streets, how="left", on=["lad11cd", "street_name2"]
    )

    ocod_data_processed["street_match"] = ocod_data_processed["is_voa_street"].fillna(
        False
    )
    ...
    final_matched_df = ocod_data_processed.merge(
        all_voa_addresses_lookup[
            ["street_name2", "street_number", "business_address", "lad11cd"]
        ].drop_duplicates(),
        how="left",
    ...
    final_matched_df.loc[
        final_matched_df["street_name2"] == "xxxstreet name missingxxx", "street_name2"
    ] = pd.NA
...
def classification_type1(ocod_data):
    ocod_data["class"] = np.select(
        [
            ...
            ocod_data["property_address"].str.contains(
            ...
            == True,  # land with words before it
            ocod_data["building_name"].str.contains(
                r"\binn$|public house|^the\s\w+\sand\s\w+|(?:tavern$)"
            )
            == True,  # pubs in various guises
    ...
    temp_fill = (
        ocod_data[
            ~ocod_data["class"].isin(["unknown", "airspace", "carpark"])
            & (ocod_data["within_larger_title"] == True)
        ]
        .groupby(["title_number", "class"])
        .size()
        .reset_index()[["title_number", "class"]]
        .drop_duplicates()
    )

    temp = ocod_data[
        ocod_data["title_number"].isin(temp_fill["title_number"])
        & (ocod_data["class"] == "unknown")
        & (ocod_data["within_larger_title"] == True)
    ].copy()
    ...
    ocod_data = pd.concat(
        [temp, ocod_data[~ocod_data["unique_id"].isin(temp["unique_id"])]]
    )
...
def classification_type2(ocod_data):
    ocod_data["class2"] = np.select(
        [
            ...
            & (
                (
                    ocod_data.property_address.str.contains("^(?:the )?unit")
                    & ocod_data.property_address.str.contains("park", regex=True)
                )
                == True
            ),  # contains the word unit and park
            (ocod_data["class"] == "unknown")
            & (
                ocod_data["business_counts"] == 0
            ),  # if there are no businesses in the oa then it is a residential
            (ocod_data["class"] == "unknown")
            & (
                ocod_data["lsoa_business_counts"] == 0
            ),  # if there are no businesses in the lsoa then it is a residential
            (ocod_data["class"] == "unknown")
            & (ocod_data["street_match"] == True)
            & (ocod_data["street_name"].notnull() == True)
            & (ocod_data["street_number"].notnull() == True),
            (ocod_data["class"] == "unknown")
            & (ocod_data["street_match"] == False)
            & (ocod_data["street_name"].notnull() == True),
            (ocod_data["class"] == "unknown")
            & (ocod_data["building_name"].notnull() == True),
        ],
        [
            "business",
            "residential",
    ...
    temp_fill = (
        ocod_data[
            ~ocod_data["class2"].isin(["unknown", "airspace", "carpark"])
            & (ocod_data["within_larger_title"] == True)
        ]
        .groupby(["title_number", "class2"])
        .size()
        .reset_index()[["title_number", "class2"]]
        .drop_duplicates()
    )

    temp = ocod_data[
        ocod_data["title_number"].isin(temp_fill["title_number"])
        & (ocod_data["class2"] == "unknown")
        & (ocod_data["within_larger_title"] == True)
    ].copy()
...
def contract_ocod_after_classification(
    ...
    ocod_data = pd.concat(
        [
            ocod_data[ocod_data[class_type].isin(classes)],
            temp.drop_duplicates(subset="title_number"),
        ]
    ...
    ocod_data = ocod_data[
        ~(
            ocod_data["unit_type"].str.contains(r"park|garage")
            & (ocod_data["class"] == "residential")
        )
    ]
