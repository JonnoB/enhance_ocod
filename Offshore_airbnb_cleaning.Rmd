...
#offshore_df <- read_csv(file.path("/home/jonno/Downloads", "offshore_df.csv"))

"(business)|(cafe)|(^shop)|( shop)|(pub)|(restaurant)"
test <- offshore_df %>%
  filter(grepl("house",offshore_df$Property_Address, ignore.case = T))
sum( grepl("(maisonette)",offshore_df$Property_Address, ignore.case = T) )
...
sum( grepl("(dolphin)",offshore_df2$Property_Address, ignore.case = T) )

  table(test$class, is.na(test$Postcode), useNA = "always")
  ...
  test <- offshore_df2 %>%
    filter(grepl("(dolphin)", Property_Address, ignore.case = T))
  
sum(grepl(  "new london", offshore_df2 $Property_Address, ignore.case = T))
