...
To ask Alexei
Make sure the time period matches the periods we are interested in
Create compound activity vectors using shorter time periods based on the population of the MSOA.
...
how_many_zero <- all_variables %>%
  ...
  summarise(low_use = sum(low_use),
            airbnb = sum(airbnb)) %>%
  left_join(all_variables_lad %>% select(-airbnb, -low_use))
