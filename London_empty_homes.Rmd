...
mean_total_value %>% ungroup %>%
  ...
  mutate(total_value = total_value/1e9) %>% 
  colClean(.) #%>% #remove the underscore for nicer tables
  xtable(., caption = "The complete metrics list for the analysis of the main property types",
...
entropy_nested <- c("nested", "normal") %>%
  map_df(~{
...
MSOAshape_nested <- all_variables_nested %>% 
  group_by(MSOA11CD) %>%
  summarise(across(.cols = c(normal, nested), .fns = sum)) %>%
  select(msoa11cd = MSOA11CD, normal, nested) %>%
  pivot_longer(., cols =c(normal, nested)) %>%
  full_join(MSOAshape_nested, .) %>%
  group_by(name) %>%
  mutate(
...
ggplot() +
  ...
  scale_fill_viridis_c() +
  labs(title = "Nested vs normal offshore domestic property", 
       fill = "log10(P)")
...
mean_total_value_nested  %>% ungroup %>%
  group_by(name) %>%
  summarise(across(.cols = c(-id), .fns =mean)) %>%
  rename(type = name) %>%
  left_join(entropy_nested) %>% left_join(morans_df_nested %>% select(-p_value), by = c("type"="name") ) %>%
  mutate(total_value = total_value/1e9) %>% 
    mutate(type = ifelse(type=="normal", "single", type)) %>%
  colClean(.) %>% #remove the underscore for nicer tables
  xtable(., caption = "The complete metrics list comparing nested and single offshore homes",
...
This section is not used as unfortuneately both the Land Registry and the VOA refuse to use the system
